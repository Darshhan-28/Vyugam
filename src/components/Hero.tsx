...
import { MapPin, Calendar, Zap, Clock, AlertTriangle, ExternalLink, Sparkles } from 'lucide-react';
  ...
  return (
    <header id="hero" className="relative min-h-screen flex flex-col items-center justify-start overflow-hidden hero-bg pb-16 sm:pb-20">
      ...
      <div className="relative z-30 flex flex-col items-center text-center mt-5 sm:mt-6 max-w-4xl px-4 w-full">
        ...
        {/* Announcement Banner for Closed Registration & AIVORA 2K26 */}
        <div className="w-full bg-obsidian/95 border-2 border-red-500 p-3.5 sm:p-4 mb-5 shadow-[5px_5px_0_#7A0606] flex flex-col sm:flex-row items-center justify-between gap-3 text-left">
          <div className="flex items-start gap-3">
            <div className="p-2 bg-red-600 text-white rounded-full flex-shrink-0 mt-0.5 shadow-[2px_2px_0_#050505]">
              <AlertTriangle className="w-5 h-5 text-yellow-300" />
            </div>
            <div>
              <span className="font-heading font-extrabold text-xs uppercase tracking-widest text-red-400 block">
                REGISTRATION CLOSED FOR VYUGAM 2.0
              </span>
              <p className="font-body text-xs sm:text-sm text-cream/90 mt-0.5">
                Pass registrations are closed. <span className="text-marigold font-bold">Missed out? Register for AIVORA 2K26!</span>
              </p>
            </div>
          </div>
          <a
            ...
            className="w-full sm:w-auto flex-shrink-0 font-heading font-extrabold text-xs uppercase tracking-wider text-obsidian bg-marigold border-2 border-obsidian px-4 py-2 shadow-[3px_3px_0_#C1121F] hover:bg-yellow-400 transition-all flex items-center justify-center gap-1.5"
          >
        ...
        <div className="flex flex-wrap gap-2.5 sm:gap-4 justify-center mt-6 sm:mt-8 w-full">
          ...
          <div className="badge-tilt-3 font-heading font-bold text-xs sm:text-sm uppercase tracking-wider bg-red-950/80 text-red-300 border-2 border-red-500 px-3.5 sm:px-4 py-2 sm:py-2.5 flex items-center gap-1.5 sm:gap-2 shadow-[3px_3px_0_#7A0606]">
            <AlertTriangle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-red-400 flex-shrink-0" /> REGISTRATION CLOSED
          </div>
        </div>

        {/* Feature Cards: Registration Status & Event Countdown */}
        <div className="mt-6 sm:mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 w-full max-w-xl sm:max-w-2xl">
          ...
          <div className="bg-carbon/90 border-2 border-red-500 p-3 sm:p-5 shadow-[5px_5px_0_#7A0606] flex flex-col items-center justify-center gap-2">
            <span className="font-mono text-[10px] sm:text-xs tracking-widest uppercase text-red-400 flex items-center gap-1.5 font-bold">
              <Clock className="w-4 h-4 text-red-500 animate-pulse" /> Registration Status
            </span>
            <div className="font-display text-2xl sm:text-3xl text-red-400 bg-obsidian border border-red-500/80 py-1 px-3 w-full text-center shadow-[3px_3px_0_#C1121F] tracking-wider uppercase font-bold">
              CLOSED
            </div>
            <span className="font-heading font-extrabold text-[10px] sm:text-xs uppercase text-marigold tracking-wider mt-0.5">
              ⚡ Pass registrations are closed
            </span>
          </div>
        ...
        {/* CTA Buttons */}
        <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center mt-6 sm:mt-8 w-full px-2 sm:px-0">
          <a
            id="hero-pass-cta"
            href={AIVORA_REGISTRATION_URL}
            target="_blank"
            rel="noopener noreferrer"
            className="font-heading font-extrabold text-sm sm:text-base lg:text-lg tracking-wider uppercase text-obsidian bg-marigold border-[3px] border-obsidian px-6 sm:px-8 py-3.5 sm:py-4 shadow-[5px_5px_0_#C1121F] hover:translate-x-[-3px] hover:translate-y-[-3px] hover:shadow-[8px_8px_0_#C1121F] btn-pulse transition-all cursor-pointer w-full sm:w-auto flex items-center justify-center gap-2"
          >
            <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-obsidian" />
          ...
          <button
            ...
            className="font-heading font-extrabold text-sm sm:text-base lg:text-lg tracking-wider uppercase text-white bg-red-700/90 border-[3px] border-obsidian px-6 sm:px-8 py-3.5 sm:py-4 shadow-[5px_5px_0_#050505] hover:bg-red-800 transition-all cursor-pointer w-full sm:w-auto flex items-center justify-center gap-2"
          >
            <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-300" />
            Registration Closed &mdash; Info
          </button>

          <a
            href="#arenas"
            className="font-heading font-bold text-sm sm:text-base lg:text-lg tracking-wider uppercase text-marigold bg-transparent border-[3px] border-marigold px-6 sm:px-8 py-3.5 sm:py-4 hover:bg-marigold hover:text-obsidian transition-colors inline-block text-center w-full sm:w-auto"
          >
            Explore The Arenas
          </a>
        </div>
        ...
        <p className="font-mono text-xs sm:text-sm text-red-400 font-bold tracking-widest uppercase mt-4">
          VYUGAM 2.0 Registration is Closed &bull; Check out AIVORA 2K26 for new registrations
        </p>
      </div>
