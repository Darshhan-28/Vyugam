// ============================================================
// VYUGAM 2.0 — Consolidated Single Vercel Serverless Function
// One entry point for all API calls (/api/*).
// Keeps deployment function count at 1 (well below Vercel Hobby limit of 12).
// ============================================================
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const pathname = urlObj.pathname.replace(/\/$/, ''); // e.g. /api/admin/registrations
  ...
  try {
    ...
    // ── 2. PUBLIC: Digital Pass Data ────────────────────────────
    if (pathname.startsWith('/api/pass') && method === 'GET') {
      // Extract token from /api/pass/:token or query token
    ...
    // ── 3. PUBLIC: Short URL QR Redirect ────────────────────────
    ...
    if (pathname.startsWith('/api/admin')) {
      ...
      // Attendance Analytics: GET /api/admin/checkins
    ...
    // ── 7. COORDINATOR: Protected Scan & Check-in ────────────────
    if (pathname.startsWith('/api/coordinator')) {
      ...
      // QR Token Scan Validation: POST /api/coordinator/scan
      if (pathname === '/api/coordinator/scan' && method === 'POST') {
      ...
      // Check-in Record Entry: POST /api/coordinator/checkin
      if (pathname === '/api/coordinator/checkin' && method === 'POST') {
